var ofmeet = (function (of) {
  ...
  const welcome = [
    "Welcome to Fastpath. How can we help you?",
    "You can type !help anytime for list of commands",
  ];
  ...
  function doConverse(credentials) {
    ...
    if (!_converse) {
      ...
      const config = {
        ...
        locales_url:
          hosted + "/widget/locale/{{{locale}}}/LC_MESSAGES/converse.json",
  ...
  function setupJitsiMeet() {
      ...
      initialize: function () {
          ...
          if (bodyElement && _converse.shouldNotifyOfMessage(data.stanza)) {
            ...
            if (pos > -1) {
              ...
              prompt.onclick = function (event) {
                ...
                var box_jid = Strophe.getBareJidFromJid(
                  chatbox.get("from") || chatbox.get("jid")
                );
        ...
        _converse.api.listen.on("renderToolbar", function (view) {
          if (view.model.get("type") == "chatroom") {
            ...
            var jitsiMeet = addToolbarItem(
              view,
              id,
              "tch" + id,
              '<a class="fas fa-video" title="Jitsi Meet"></a>'
            );

            jitsiMeet.addEventListener(
              "click",
              function (evt) {
                evt.stopPropagation();

                var jitsiConfirm =
                  _converse.api.settings.get("jitsimeet_confirm");
        ...
        MeetDialog = _converse.BootstrapModal.extend({
          initialize() {
            _converse.BootstrapModal.prototype.initialize.apply(
              this,
              arguments
            );
          ...
          toHTML() {
            ...
            return (
              '<div class="modal" id="myModal"> <div class="modal-dialog modal-lg"> <div class="modal-content">' +
              ...
              "</h4>" +
              "</div>" +
              '<div style="text-align: center;" class="modal-body"><iframe src="' +
              url +
              '" id="jitsimeet" allow="microphone; camera;" frameborder="0" seamless="seamless" allowfullscreen="true" scrolling="no" style="z-index: 2147483647;width:640px;height:480px;display: inline-block"></iframe></div>' +
      ...
      overrides: {
        MessageView: {
          renderChatMessage: async function renderChatMessage() {
            var dataJid = Strophe.getBareJidFromJid(
              this.model.get("from") || this.model.get("jid")
            );
            ...
            if (pos > -1) {
              ...
              var link_id =
                link_room + "-" + Math.random().toString(36).substr(2, 9);
              var link_label =
                pos > 0
                  ? body.substring(0, pos)
                  : _converse.api.settings.get("jitsimeet_invitation");
    ...
    var setupContentHandler = function (
      chat,
      avRoom,
      content,
      linkId,
      ...
      if (chat.model.vcard) {
        msg_content.innerHTML =
          ...
          chat.model.vcard.attributes.image +
          '" style="width: 36px; width: 36px; height: 100%; margin-right: 10px;"/> <div class="chat-msg-content"> <span class="chat-msg-heading"> <span class="chat-msg-author">' +
          chat.model.getDisplayName() +
          '</span> <span class="chat-msg-time">' +
          pretty_time +
          '</span> </span> <span class="chat-msg-text">' +
          content +
      ...
      if (avRoom && linkId) {
        setTimeout(function () {
          if (document.getElementById(linkId))
            document.getElementById(linkId).onclick = function (evt) {
              var url =
                _converse.api.settings.get("jitsimeet_url") + "/" + avRoom;
    ...
    var doVideo = function doVideo(view) {
      var room =
        Strophe.getNodeFromJid(view.model.attributes.jid).toLowerCase() +
        "-" +
    ...
    var doLocalVideo = function doLocalVideo(view, room, url, label) {
      ...
      if (modal) {
        meetDialog = new MeetDialog({
          model: new converse.env.Backbone.Model({
            url: url,
            view: view,
            label: label,
            room: room,
        ...
        if (div) {
          div.innerHTML =
            '<iframe src="' +
            url +
            '" id="jitsimeet" allow="microphone; camera;" frameborder="0" seamless="seamless" allowfullscreen="true" scrolling="no" style="z-index: 2147483647;width:100%;height:-webkit-fill-available;height:-moz-available;"></iframe>';
    ...
    var openChatbox = function openChatbox(view) {
      ...
      if (jid) {
        ...
        else if (type == "chatroom") _converse.api.rooms.open(jid);
      }
    };
  }
      ...
      initialize: function () {
        ...
        var QRCodeDialog = _converse.BootstrapModal.extend({
          ...
          toHTML() {
            ...
            return (
              ...
              '<div class="modal-header"><h1 class="modal-title">' +
              title +
              '</h1><button type="button" class="close" data-dismiss="modal">&times;</button></div>' +
          ...
          afterRender() {
            ...
            this.el.addEventListener(
              ...
              function () {
                if (qrcode) {
                  that.el.querySelector(".modal-body").innerHTML =
                    '<img src="' + qrcode + '" />';
                }
              },
              false
          ...
          startSession() {
            this.el.querySelector(".modal-status").innerHTML =
              "Please follow the instructions in your IRMA app";
          },
          endSession() {
            this.modal.hide();
          },
        ...
        _converse.api.listen.on("chatRoomOpened", function (view) {
          console.debug(
            ...
            _converse.nickname,
            view.model.get("nick"),
            _converse.chatboxviews.model.models.length
          );
          document.getElementById("chatloader").style.visibility = "hidden";
        });
        ...
        _converse.api.listen.on("renderToolbar", function (view) {
          ...
          if (view.model.get("type") == "chatbox") {
          ...
          } else if (view.model.get("type") == "chatroom") {
            var id = view.model.get("box_id");
            var jid = view.model.get("jid");

            var scrolldown = addToolbarItem(
              view,
              id,
              "ignite-scrolldown-" + id,
              '<a class="fa fa-angle-double-down" title="Scroll to the bottom"></a>'
            );
            ...
            var screencast = addToolbarItem(
              view,
              id,
              "ignite-screencast-" + id,
              '<a class="fa fa-desktop" title="ScreenCast. Click to here to start and click here again to stop"></a>'
            );

            screencast.addEventListener(
              "click",
              function (evt) {
                evt.stopPropagation();
        ...
        MessageView: {
          renderChatMessage: async function renderChatMessage() {
            if (this.model.vcard) {
              ...
              if (
                nick &&
                _converse.DEFAULT_IMAGE == this.model.vcard.attributes.image
              ) {
      ...
      initialize: function () {
        ...
        window.click2Dial = {
          ...
          sip: {
            domain: domain,
            server:
              "wss://" + server + "/sip/proxy?url=ws://" + domain + ":5066",
            register: false,
            caller_uri: "sip:1002@" + domain,
            authorization_user: "1002",
            password: "1234",
          },
          xmpp: {
            domain: "meet.jit.si",
            server: "https://meet.jit.si/http-bind",
          },
        };
      ...
      overrides: {
        ChatBoxView: {
          toggleCall: function toggleCall(ev) {
            ev.stopPropagation();
            var room = converse.env.Strophe.getNodeFromJid(
              this.model.attributes.jid
            ).toLowerCase();
            console.debug("toggleCall", room);

            if (
              !voxbone.WebRTC.rtcSession.isEnded ||
              voxbone.WebRTC.rtcSession.isEnded()
            ) {
              ...
              this.showHelpMessages([
                "Active call in progress. Clear call and try again",
              ]);
            }
          },
        },
  ...
  function addToolbarItem(view, id, label, html) {
    ...
    if (!placeHolder) {
      ...
      smiley.insertAdjacentElement(
        "afterEnd",
        __newElement("li", "place-holder")
      );
  ...
  function handleStream(stream, view) {
    navigator.mediaDevices
      ...
      .then((audioStream) => handleAudioStream(stream, audioStream, view))
  ...
  function handleAudioStream(stream, audioStream, view) {
    ...
    setTimeout(function () {
      ...
      videoRecorder.onstop = function (e) {
        ...
        var file = new File(
          [blob],
          "screencast-" + Math.random().toString(36).substr(2, 9) + ".webm",
  ...
  of.doBadge = function doBadge(count) {
    ...
    var unreadMessageIndicator = document.getElementById(
